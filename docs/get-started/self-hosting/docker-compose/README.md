...
# The api, zitadel-init and runner services read these three files directly, so
# they must exist before Compose starts even when you configure nothing in them.
# Leave them as-is for a plain installation; fill them in when you add SSO, a VCS
# provider, or cloud workload identity.
cp sso.env.example sso.env
cp vcs.env.example vcs.env
cp oidc.env.example oidc.env
...
| `sso.env` | SSO / external IdP credentials | Copy from `sso.env.example`; edit with your SSO provider values, or leave empty |
| `vcs.env` | VCS provider OAuth credentials | Copy from `vcs.env.example`; edit with your VCS provider values, or leave empty |
| `oidc.env` | OIDC workload identity signing key | Copy from `oidc.env.example`; edit with your OIDC key and issuer, or leave empty |

The contents of `sso.env`, `vcs.env` and `oidc.env` are optional, but the files themselves are not: Compose reads them as `env_file` entries and refuses to start the stack if any is missing. Copy all three from their `.example` counterparts even for a plain installation.
