...
| Variable | Description | Default |
|---|---|---|
| `STORAGE_BACKEND` | Storage provider (`minio` or `s3`) | `minio` |
| `STORAGE_BUCKET` | Bucket for Terraform module registry | `terraform-registry` |
| `MINIO_ENDPOINT` | MinIO/S3 endpoint (host:port) | `localhost:9000` |
| `MINIO_ACCESS_KEY` | Access key / AWS access key ID | `minioadmin` |
| `MINIO_SECRET_KEY` | Secret key / AWS secret access key | `minioadmin` |
| `MINIO_USE_SSL` | Use HTTPS for MinIO connection | `false` |
...
| `VITE_ZITADEL_ISSUER` | Zitadel issuer URL as seen by the browser | `http://localhost:8080` |
...
| `ZITADEL_LOGIN_UI_BASE_URL` | Login UI base URL | `http://localhost:3000/ui/v2/login` |
| `ZITADEL_LOGIN_UI_ORIGIN` | Login UI origin for CORS | `http://localhost:3000` |
...
| Variable | Description | Default |
|---|---|---|
| `ZITADEL_ISSUER` | Zitadel issuer URL (with `/ui/v2/login` suffix) | `http://localhost:8080/ui/v2/login` |
...
| Variable | Description | Default |
|---|---|---|
| `NEXT_PUBLIC_BASE_PATH` | Base path for the login UI | `/ui/v2/login` |
| `ZITADEL_API_URL` | Internal Zitadel API URL | `http://localhost:8080` |
| `ZITADEL_SERVICE_USER_TOKEN` | Service user PAT for Zitadel API access | Auto-generated by `zitadel-init` |
| `EMAIL_VERIFICATION` | Enable email verification | `false` |
| `CUSTOM_REQUEST_HEADERS` | Headers sent to Zitadel (for external domain routing) | (none) |
