...
> **Implementation status - 2026-08-27:** Live on all eight satellites - [`api`](https://github.com/vhco-pro/stackweaver-api), [`frontend`](https://github.com/vhco-pro/stackweaver-frontend), [`orchestrator`](https://github.com/vhco-pro/stackweaver-orchestrator), [`zitadel-init`](https://github.com/vhco-pro/stackweaver-zitadel-init), [`secrets-init`](https://github.com/vhco-pro/stackweaver-secrets-init), [`helm`](https://github.com/vhco-pro/stackweaver-helm), [`ansible-runner`](https://github.com/vhco-pro/stackweaver-ansible-runner), and [`opentofu-runner`](https://github.com/vhco-pro/stackweaver-opentofu-runner). Each rollout was verified by an end-to-end smoke test: monorepo push opens a `sync/<sha>` PR authored by `stackweaver-release-bot[bot]`, the four hard gates in `auto-approve-sync.yml` pass, `stackweaver-pr-reviewer[bot]` approves, the PR squash-merges after CodeQL goes green, and the existing GitVersion → Release cascade attaches `provenance.intoto.jsonl`, `sbom.spdx.intoto.jsonl`, and `checksums.txt` to the new tag. The former exclusion, the OpenTofu runner, joined the model once its rewrite completed and the repository went public - see [§ Formerly excluded: the OpenTofu runner](#formerly-excluded-the-opentofu-runner) below.

Stackweaver is developed as a private monorepo at `michielvha/stackweaver`. Each user-facing component is mirrored to its own public satellite repository under the `vhco-pro` organisation (`stackweaver-api`, `stackweaver-frontend`, `stackweaver-orchestrator`, `stackweaver-zitadel-init`, `stackweaver-secrets-init`, `stackweaver-ansible-runner`, `stackweaver-helm`, and `stackweaver-opentofu-runner`). The satellites are what users build, audit and depend on; the monorepo is what we develop in.
...
Before the PR-based sync hardening described above, the monorepo sync workflows used the release-bot App to push directly to the satellite `main` branch with no review and no PR. That model had three problems: a single compromised App key was sufficient for full satellite compromise, no change had a queryable review trail, and CI ran only after the merge. The PR-based model is strictly more secure on all three counts. The cost is roughly three to five minutes of added end-to-end latency per sync and one additional App to manage credentials for.

## Formerly excluded: the OpenTofu runner

The runner satellite was the one repository excluded from D-SYNC-PR, and it no longer is. The exclusion was forced by a GitHub plan constraint rather than a design choice: under the GitHub Free plan neither branch protection nor `allow_auto_merge` is available on private repositories, and both are load-bearing for D-SYNC-PR's enforcement boundary. The repository was private because its image bundled the Terraform CLI, which the project will not redistribute under BUSL-1.1.

That blocker is resolved. The runner was rewritten on OpenTofu (MPL-2.0), and the satellite is now [`stackweaver-opentofu-runner`](https://github.com/vhco-pro/stackweaver-opentofu-runner) - public, and on the same footing as the other seven: it ships the same `auto-approve-sync.yml` template, is synced by the branch-push/PR model in `sync-runner.yml`, and carries the same branch protection (required reviews plus a required status check). The legacy private `stackweaver-runner` repository is retired, referenced by no workflow, and will not be opened.

With the exclusion closed, no satellite is on the legacy direct-push model and the weaker pre-hardening threat model no longer applies anywhere in the fleet.
...
# stackweaver-helm, stackweaver-opentofu-runner
