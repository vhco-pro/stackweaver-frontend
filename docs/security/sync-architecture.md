...
Until then, the security guarantees on `stackweaver-runner` are weaker than on the other satellites: a compromised release-bot key could push directly to `main` without any approval, review trail, or PR-side CI. That is the same threat model that the other six satellites had before this PR-based hardening landed. The runner therefore inherits the *old* threat model and is documented as such in the internal audit binder; it does not change the threat model of the other six satellites.
