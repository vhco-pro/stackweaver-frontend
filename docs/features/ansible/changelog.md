...
### Changed
- **The run viewer uses the width of a large display**: the whole application used to render inside a fixed 1280px column, so on a 2560px screen the host-by-task matrix scrolled horizontally while roughly 490px sat empty on either side of it. Pages now declare how much room their content wants, and the run viewer asks for the wider tier: a thirteen-task run fits without scrolling, Host Facts shows four hosts side by side instead of two by two, and the Details tab lays out on two rows instead of four. The Inventories, Collections and Credentials grids take the same tier and answer the extra room with more cards per row rather than wider cards. Sparse tables such as the job list keep the original width on purpose, because spreading five short columns across a 4K display only lengthens the distance between a row's name and its status. Nothing changes at or below 1440px, and mobile is untouched. See `frontend/src/components/layout/PageContainer.tsx`.
- **One credential set per template and per job**: templates and jobs each carried both a single `credential_id` and, on templates, a separate multi-credential set - two places recording the same thing, which is what let a template display "No credentials attached" while using one. The single field is gone from both. A template now has exactly one credential set, and launching **snapshots that set onto the job**, so the job records what it actually ran with: editing a template no longer changes an already-launched job, and a relaunch reproduces the original run rather than picking up whatever the template holds today. The API relationship is `credentials` (an array) on both resources; on update, sending it replaces the set and an empty array clears it, while omitting it leaves the set alone. Existing templates and jobs are migrated into their sets on startup and the old columns dropped. In Terraform, `credential_id` is removed from `stackweaver_ansible_job_template` - attach credentials with `stackweaver_ansible_job_template_credential`. See `core/models/ansible_job.go` and `docs/features/ansible/api-reference.md`.

### Fixed
- **A job template's credential shows up on the template**: creating or editing a template through the API sets its credential relationship, but only the multi-credential attachment set is listed on the template page - and nothing ever wrote the one from the other. A template created over the API therefore reported "No credentials attached" while holding, and using, a perfectly good credential. Both paths now write the attachment set, reassigning a credential swaps it rather than accumulating a second one, and clearing it detaches. Existing templates are reconciled on startup, leaving any set the user curated themselves untouched. Runs were never affected - the runner already read both. See `backend/internal/api/v2/handlers/ansible/playbooks.go`.
- **A job can no longer ride another job's SSH connection**: the runner enabled SSH connection multiplexing but pointed every job at one shared control-socket directory, so OpenSSH would hand a job a live, already-authenticated connection opened by a different job to the same host and user. A job whose credential the target rejects therefore ran successfully for as long as that socket stayed alive, which on the shared platform runner meant one organization could reach a host it holds no valid key for. Each job now gets its own control-socket namespace, so a credential the target rejects fails as it should, with every host reported unreachable. Multiplexing within a single job is unchanged. See `backend/cmd/ansible-runner/main.go`.
- **`hosts-total` reports the real host count**: the field was always zero on every job the platform ran, because nothing ever wrote it - the only function that did was never called from anywhere, while both ingest paths set the other seven counters directly. Anything reading the job resource to size a run, or computing a success ratio against the total, saw zero. Both ingest paths now take the count from the play recap, which is keyed by host and is the authoritative source. See `backend/cmd/ansible-runner/main.go` and `backend/internal/api/v2/handlers/runner_agent.go`.
