...
import { teamsApi, type TeamToken as TeamTokenModel } from '@/api/client';
...
/**
 * TeamToken manages a team's single API token (tfe_team_token): an automation credential that acts as
 * the team. It shows the token metadata (created / last used / expiry), lets an org owner generate or
 * regenerate it, and reveals the secret value exactly once. Data fetching uses React Query. Mirrors the
 * show-once pattern of OrganizationToken. Only the legacy (descriptionless) single-token-per-team path
 * is supported.
 */
export function TeamToken({ teamId }: { teamId: string }) {
  const queryClient = useQueryClient();
  const [revealed, setRevealed] = useState<TeamTokenModel | null>(null);

  const { data: token, isLoading } = useQuery({
    queryKey: ['teamToken', teamId],
    queryFn: () => teamsApi.getToken(teamId),
    enabled: !!teamId,
  });

  const createMutation = useMutation({
    mutationFn: () => teamsApi.createToken(teamId),
    onSuccess: (created) => {
      setRevealed(created);
      void queryClient.invalidateQueries({ queryKey: ['teamToken', teamId] });
    },
    onError: (err: unknown) => {
      toast.error(err instanceof Error ? err.message : 'Failed to generate team token');
    },
  });

  const deleteMutation = useMutation({
    mutationFn: () => teamsApi.deleteToken(teamId),
    onSuccess: () => {
      setRevealed(null);
    ...
    onError: (err: unknown) => {
      toast.error(err instanceof Error ? err.message : 'Failed to revoke team token');
    },
  });
  ...
  const regenerate = () => {
    if (token && !confirm('Regenerate the team token? The current token will stop working immediately.')) return;
    createMutation.mutate();
  };

  const revoke = () => {
    if (!confirm('Revoke the team token? Any automation using it will stop working immediately.')) return;
      ...
      {revealed?.token && (
        <div className={cn(
          ...
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="text-lg font-semibold text-green-400">Team token generated</h3>
          </div>
