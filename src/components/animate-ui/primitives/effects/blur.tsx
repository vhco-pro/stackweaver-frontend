...
function Blur({
  ...
  return (
    <Component
      ...
      transition={{
        ...transition,
        delay: (typeof transition?.delay === 'number' ? transition.delay : 0) + delay / 1000,
      }}
      {...props}
    />
  );
  ...
  return (
    <>
      {array.map((child, index) => {
        const holdDelayNum = typeof holdDelay === 'number' ? holdDelay : 0;
        const indexNum = typeof index === 'number' ? index : 0;
        const delayNum = typeof delay === 'number' ? delay : 0;
        const calculatedDelay = delayNum + (indexNum * holdDelayNum);
        return (
          <Blur
            key={child.key ?? index}
