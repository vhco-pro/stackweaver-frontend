  ...
  return (
    ...
    >
      {tokens.map((tok, wi) => {
        ...
        // Calculate word delay based on current index (avoid mutating globalIndex during render)
        const currentIndex = tokens.slice(0, wi).reduce((acc, t) => acc + Array.from(t).length, 0);
        const wordDelay = baseDelaySec + perChar * currentIndex;
